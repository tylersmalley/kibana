...
import { fromRoot, exec } from '../../utils';
...
export async function rebuildCache(settings, logger) {
  ...
  await exec(logger, kibanaScript, kibanaArgs, {
    cwd: fromRoot('.'),
    level: 'log',
...
export function assertVersion(settings) {
  ...
  if (!versionSatisfies(actual, expected)) {
    throw new Error(
      `Incorrect Kibana version in plugin [${settings.plugins[0].name}]. ` +
