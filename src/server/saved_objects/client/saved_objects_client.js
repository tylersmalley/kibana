...
export class SavedObjectsClient {
  ...
  async _withKibanaIndex(method, params) {
    try {
      const esResponse = await this._callWithRequest(this._request, method, {
        ...params,
        index: this._kibanaIndex,
      });

      const data = get(esResponse, 'hits.hits', []).map(r => {
        return Object.assign(r._source, { id: r._id, type: r._type });
      });

      return {
        data,
        total: get(esResponse, 'hits.total')
      };
    } catch (err) {
