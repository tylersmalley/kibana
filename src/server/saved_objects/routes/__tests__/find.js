...
describe('GET /api/kibana/saved_objects/{type?}', () => {
  ...
  it('calls upon savedObjectClient.find with defaults', async () => {
    ...
    expect(options).to.eql({ size: 20, from: 0 });
  });

  it('accepts the query parameter size/from', async () => {
    const request = {
      method: 'GET',
      url: '/api/kibana/saved_objects?size=10&from=50'
    };
    ...
    const options = savedObjectsClient.find.getCall(0).args[0];
    expect(options).to.eql({ size: 10, from: 50 });
  });
  ...
  it('accepts the query parameter fields as a string', async () => {
    ...
    const options = savedObjectsClient.find.getCall(0).args[0];
    expect(options).to.eql({ size: 20, from: 0, fields: 'title' });
  });
  ...
  it('accepts the type as a query parameter', async () => {
    ...
    const options = savedObjectsClient.find.getCall(0).args[0];
    expect(options).to.eql({ size: 20, from: 0, type: 'index-pattern' });
  });
