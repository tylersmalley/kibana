...
can run `node scripts/lint --fix` to fix linting issues and apply formatting.
...
able to be represented as a directed acyclic graph. To achieve that, we must avoid circular dependencies
in code and between type imports. One of the most critical parts is the plugins
code.

When building plugins, avoid importing from plugins that have circular dependencies as well as introducing new ones.

**Important notes about circular dependency detection:**

- **TypeScript doesn't catch circular dependencies between files at compile time.** Be aware that TypeScript (and ES6 modules) do allow circular imports between files, and circular dependencies will compile successfully, but can, and often do, cause runtime issues.

- **TypeScript project references can detect circular dependencies between projects** when building with the TypeScript compiler. If you're using TypeScript project references (see [TypeScript documentation](docs/extend/typescript.md)), circular dependencies between projects will be caught during the build process. You can check this by running `node scripts/type_check` or `./node_modules/.bin/tsc -b` on your project.

- **The Kibana plugin system detects circular dependencies at runtime.** When plugins load, the system performs topological sorting and will throw on detecting circular dependencies.

- **For TypeScript project references**, circular dependencies have to be resolved because they prevent proper type declaration generation.
