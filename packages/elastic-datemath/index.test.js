...
// jest.spyOn(global.Date, 'now').mockImplementationOnce(() => new Date(anchor));
...
describe('dateMath', function () {
  ...
  beforeEach(() => jest.spyOn(global.Date, 'now').mockImplementationOnce(() => new Date(anchor)));
  ...
  describe.only('objects and strings', function () {
    const now = moment();
    const mmnt = moment(anchor);
    const date = mmnt.toDate();
    const string = mmnt.format(format);
  ...
  describe('math and rounding', function () {
    ...
    it('should parse long expressions', () => {
      ...
      expect(dateMath.parse('now-1d/d+8h+50m').format(format)).toBe(
        '2013-12-30T08:50:00.000-08:00'
      );
    });
  });
  ...
  describe('used momentjs instance', function () {
    it('should not use default moment instance if parameter is specified', function () {
      const m = { isMoment: jest.fn(() => true) };
      const pared = dateMath.parse('now', { momentInstance: m });
      expect(m.isMoment).toBeCalled();
      expect(pared).toBe('now');
    });

    it('should work with multiple different instances', function () {
      const m1 = { isMoment: jest.fn(() => true) };
      const m2 = { isMoment: jest.fn(() => true) };

      dateMath.parse('now', { momentInstance: m1 });
      expect(m1.isMoment).toBeCalled();
      expect(m2.isMoment).not.toBeCalled();

      jest.clearAllMocks();

      dateMath.parse('now', { momentInstance: m2 });
      expect(m1.isMoment).not.toBeCalled();
      expect(m2.isMoment).toBeCalled();
    });

    it('should use global instance after passing an instance', function () {
      const m = { isMoment: jest.fn(() => true) };

      let format = dateMath.parse('now', { momentInstance: m });
      expect(m.isMoment).toBeCalled();
      expect(format).toBe('now');

      format = dateMath.parse('now');
      expect(format).not.toBe('now');
    });
  });
